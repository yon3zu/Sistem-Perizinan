...
@section('breadcrumb', 'Perbarui data karyawan')

@section('content')
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="bg-teal-600 px-6 py-4">
            <h2 class="text-white font-bold text-lg">Edit Data Karyawan</h2>
            <p class="text-teal-200 text-sm">{{ $karyawan->nama }} &bull; {{ $karyawan->nip }}</p>
        </div>

        <form method="POST" action="{{ route('admin.karyawan.update', $karyawan->id) }}" class="p-6 space-y-4">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select name="team_id"
                    ...
                    <option value="{{ $team->id }}" {{ old('team_id', $karyawan->team_id) == $team->id ? 'selected' : '' }}>
                        {{ $team->nama }}
                    </option>
                    @endforeach
                </select>
