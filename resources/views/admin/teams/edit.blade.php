...
@section('breadcrumb', 'Perbarui informasi tim')

@section('content')
<div class="max-w-md mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-purple-600 to-purple-700 px-6 py-4">
            <h2 class="text-white font-bold text-lg">Edit Tim</h2>
            <p class="text-purple-100 text-sm">{{ $team->nama }}</p>
        </div>
        <form method="POST" action="{{ route('admin.teams.update', $team->id) }}" class="p-5 space-y-4">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                    Nama Tim <span class="text-red-500">*</span>
                </label>
                <input type="text" name="nama" value="{{ old('nama', $team->nama) }}"
